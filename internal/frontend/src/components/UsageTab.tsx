...
const normalizeStats = (value: UsageStatsResponse | null): UsageStatsResponse | null => {
    if (!value) {
        return null;
    }
    return {
        ...value,
        by_source_type: asArray(value.by_source_type),
        users_per_minute: asArray(value.users_per_minute),
        requests_per_minute: asArray(value.requests_per_minute),
    };
};
...
export default function UsageTab() {
    ...
    const usageRows = asArray<Usage>(usages);
    const dailyUsageRows = asArray<DailyUsage>(dailyUsages);
    const usageStats = normalizeStats(stats);

    const loadUsages = async () => {
        try {
            setLoading(true);
            const data = await usageAPI.list({ page, size });
            setUsages(asArray<Usage>(data?.items));
    ...
    const handleDailyQuery = async () => {
        ...
        try {
            ...
            if (startDate && endDate) {
                ...
                setDailyUsages(asArray<DailyUsage>(resp?.data?.daily_usages));
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const details = asArray<Usage>(resp?.details);
            setUsages(details);
    ...
    return (
        <div>
            ...
            {usageStats && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6 mb-6">
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div className="text-3xl font-bold text-rose-600">{usageStats.peak_users?.toLocaleString() || '0'}</div>
                        <div className="text-sm text-gray-500 mt-1">
                            {usageStats.peak_users_date}
                            {usageStats.peak_users_minute ? ` @ ${usageStats.peak_users_minute}` : ''}
                        </div>
                    </div>
                    <div className="bg-white rounded-lg shadow p-6">
                        <div className="text-sm text-gray-600 mb-1">当天最大并发请求数</div>
                        <div className="text-3xl font-bold text-orange-600">{usageStats.peak_requests?.toLocaleString() || '0'}</div>
                        <div className="text-sm text-gray-500 mt-1">
                            {usageStats.peak_users_date}
                            {usageStats.peak_requests_minute ? ` @ ${usageStats.peak_requests_minute}` : ''}
                        </div>
                    </div>
                    <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div className="text-3xl font-bold text-purple-600">{usageStats.record_count ?? total}</div>
                    </div>
                    {usageStats.by_source_type.map((s) => (
            ...
            {/* 每分钟统计（用于查看当天峰值） */}
            {usageStats && usageStats.users_per_minute.length > 0 && (
                <div className="bg-white rounded-lg shadow overflow-hidden mb-6">
                    <div className="p-4 border-b">
                        <div className="flex items-center justify-between">
                            <div className="font-medium">每分钟使用人数</div>
                            <div className="text-sm text-gray-600">
                                {usageStats.peak_users_date} 峰值 {usageStats.peak_users?.toLocaleString() || '0'} 人
                            </div>
                        </div>
                    </div>
                    <div className="overflow-x-auto max-h-96 overflow-y-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {usageStats.users_per_minute.map((m) => (
                                    <tr key={m.minute} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-3 whitespace-nowrap text-sm font-medium text-indigo-600">
                                            {Number(m.user_count ?? 0).toLocaleString()}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
